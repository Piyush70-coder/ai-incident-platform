...
from incidents.tasks import process_incident_logs, generate_incident_embedding, generate_postmortem_report
...
def handle_log_ingestion(*, company, user, incident_id, log_text, uploaded_file):
    ...
    with transaction.atomic():
        if incident_id:
            ...
            transaction.on_commit(lambda iid=_iid: process_incident_logs.delay(iid, False))
        ...
        def _after_new():
            ...
            generate_postmortem_report.apply_async(args=[str(incident.id)], countdown=12)
